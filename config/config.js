...
var home = process.env.INSIGHT_DB || (getUserHome() + '/.chratos-insight');

if (process.env.INSIGHT_NETWORK === 'livenet') {
  env = 'livenet';
  db = home;
  port = '3000';
  b_port = '45891'; // RPCPort
  p2p_port = '46891';
} else {
  env = 'testnet';
  db = home + '/testnet';
...
if (!dataDir) {
  if (isWin) dataDir = '%APPDATA%\\Chratos\\';
  if (isMac) dataDir = process.env.HOME + '/Library/Application Support/Chratos/';
  if (isLinux) dataDir = process.env.HOME + '/.chratos/';
}
